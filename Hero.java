...
public final class Hero extends DynamicThings {
    ...
    /**
     * Increase the health of the hero by the specified amount, up to a maximum of 100.
     * Used when the user passes through food.
     *
     * @param amount The amount by which to increase the hero's health.
     */
    ...
    public void moveIfPossible(double dx, double dy, Dungeon dungeon) {
        ...
        for (Things thing : dungeon.getRenderList()) {
            ...
            if (thing instanceof Food) {
                if(((Food) thing).getHitBox().intersect(this.getHitBox())){
                    dungeon.removeFromRenderList(thing);
                    if (health < 100) {
                        increaseHealth(10);
                    }
                    break;
